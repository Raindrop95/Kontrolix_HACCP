...
export default {
  ...
  methods: {
    ...
    showLogImage(logId) {
      kontrolix_api.getLocationLogImage(logId).then((response) => {
  ...
  async mounted() {
    await kontrolix_api.getLocationLogs(this.selectedQrCode.qr_code_id).then((response) => {
